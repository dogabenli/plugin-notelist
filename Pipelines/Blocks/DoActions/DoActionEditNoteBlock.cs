...
namespace Plugin.Sample.NoteList.Pipelines.Blocks.DoActions
{
    ...
    public class DoActionEditNoteBlock : PipelineBlock<EntityView, EntityView, CommercePipelineExecutionContext>
    {
        private readonly CommerceCommander _commerceCommander;

        public DoActionEditNoteBlock(CommerceCommander commerceCommander)
        {
        ...
        public override Task<EntityView> Run(EntityView arg, CommercePipelineExecutionContext context)
        {
            ...
            if (string.IsNullOrEmpty(arg.Action) || !arg.Action.Equals(notesActionsPolicy.EditNote, StringComparison.OrdinalIgnoreCase))
            {
            ...
            var noteComponent = listComponent.NoteListComponents[arg.ItemId];

            noteComponent.NoteText = arg.Properties.FirstOrDefault(x => x.Name.Equals(nameof(NoteComponent.NoteText), StringComparison.OrdinalIgnoreCase))?.Value;
            noteComponent.EditedOnUtc = DateTime.UtcNow;
